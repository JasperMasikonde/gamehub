...
export function FeeRuleForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setLabel(""); setMinWager(""); setMaxWager(""); setFee(""); setTransactionFee("");
      router.refresh();
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-bg-surface border border-bg-border rounded-2xl p-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-3">
        ...
        <div>
          <label className="block text-xs text-text-muted mb-1">Transaction Fee (KES)</label>
          <input type="number" min="0" step="0.01" value={transactionFee} onChange={e => setTransactionFee(e.target.value)} placeholder="10" className={cls + " w-full"} />
        </div>
      </div>
      <p className="text-xs text-text-muted mb-3">
        Winner receives: (wager × 2) − platform fee − transaction fee.
        Platform fee = your profit. Transaction fee = M-Pesa payout cost.
      </p>
